...
interface ContentRepository : Neo4jRepository<ContentNode, String>{
    ...
    @Query("""
        ...
        // 对于更深层次的关系，使用可变长度路径
        OPTIONAL MATCH deeperPath = (c)-[:REFERENCES|CONTAINS*1..${'$'}depth]->(relatedContent:Content)<-[:DESCRIBES]-(k2:Keyword)
        WHERE k2.keyword IN ${'$'}keywords AND c <> relatedContent
        
        WITH c, directKeywords, directMatches,
             collect(DISTINCT relatedContent) AS relatedContents,
